...
class DocType(TransactionBase):
	...
	def on_update(self):
		sp=webnotes.conn.get_value('Sales Person',{'employee':self.doc.employee},'name')
		if sp:
			sp_vp=webnotes.conn.sql("select name,variable_pay from `tabSales Person Variable Pay` where sales_person='%s' and fiscal_year='%s' and month='%s' and status='False'"%(sp,self.doc.fiscal_year,self.doc.month),as_list=1)
			if sp_vp:
				self.doc.gross_pay=cstr(cint(self.doc.gross_pay)+cint(sp_vp[0][1]))
				self.doc.net_pay=cstr(cint(self.doc.net_pay)+cint(sp_vp[0][1]))
				self.doc.variable_pay=cstr(sp_vp[0][1])
				self.doc.save()
				webnotes.conn.sql("update `tabSalary Slip Earning` set e_amount='%s',e_modified_amount='%s' where e_type='Variable Pay' and parent='%s'"%(self.doc.variable_pay,self.doc.variable_pay,self.doc.name))
				webnotes.conn.sql("Update `tabSales Person Variable Pay` set status='True' where name ='%s'"%(sp_vp[0][0]))
	...
	def calculate_ded_total(self):
		...
		#w=webnotes.conn.sql("update `tabLoan Installment Details` set status='Paid' where date_of_installment=%s",getdate(i[0]),as_list=1)
